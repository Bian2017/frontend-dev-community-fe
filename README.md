...
writing() {
  this.fs.copy(
    // 把所有templates下面的文件到复制到新的generators里面
    this.templatePath('**'),
    // ./表示当前目录
...
/*
*  在安装过的过程中，会提示"Please install bower with npm instal -g bower and try again."
* 目前官方已不再推荐 bower 安装依赖包，此时需关闭它。
*/
install() {
  // 关闭bower
